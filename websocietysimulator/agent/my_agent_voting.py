...
###########################################################
# 5. Persona Builder (multi-sample + voting)
###########################################################
class PersonaBuilder:
    """
    Persona extraction with:
    - sampling K reviews
    - multiple LLM calls
    - majority voting over key fields

    最终输出一个 Python dict (persona JSON)，Reasoner 会直接消化。
    """

    def __init__(
        self,
        llm: LLMBase,
        persona_votes: int = 3,   # 可调：>1 表示多次采样 + 投票
        sample_size: int = 20,
        debug: bool = False,
    ):
        self.llm = llm
        self.persona_votes = persona_votes
    ...
    # sample up to sample_size reviews
    def _sample_reviews(self, reviews: List[str]) -> List[str]:
        if len(reviews) <= self.sample_size:
            return reviews
        return random.sample(reviews, self.sample_size)

    # persona prompt (简化 schema，适合 voting)
    def _persona_prompt(
        self,
        ...
        TASK
        Infer the user's core persona features using ONLY the provided reviews.
        Be concise. Avoid unnecessary details. Generate one user sample review for the item.

        DEFINITIONS:
        tone: casual / blunt / enthusiastic / muted / formal
        sentence_length: short / medium / long
        review_length: short / medium / long
        tendency: generous / neutral / harsh
        expected_sentiment: positive / neutral / negative
        current_domain_expertise: expert / intermediate / novice / none

        current_domain: domain of this item:
            - amazon products → electronics, gaming, musical_instruments, household, accessories, tools, etc.
            - yelp → restaurant, cafe, hotel, local_service, salon, auto, dentist, etc.
            - goodreads → fiction, nonfiction, fantasy, romance, thriller, philosophy, biography, etc.
        interest_focus of current domain:
            Examples:
            - price, durability, reliability
            - taste / flavor (for restaurants)
            - story / characters (books)
            - comfort, ease_of_use, portability
        utility_focus: high / medium / low
        aesthetic_focus: high / medium / low
        experience_focus: high / medium / low

        IMPORTANT:
        "item_sentiment_prior" MUST reflect the user's expected attitude TOWARD THIS SPECIFIC ITEM ONLY.
        "current_domain_expertise" MUST describe the user's expertise level IN THE DOMAIN OF THIS ITEM, not global expertise.
        Do NOT generalize these fields; they MUST be computed based solely on this task's item_info and the user's past behavior in similar domains.
        ...
        STRICT OUTPUT SCHEMA:
            ...
            "item_sentiment_prior": {{
                "expected_sentiment": ""
            }},
            "sample_review": "<sample review sentence>",
    ...
    def _vote_field(self, persona_list: List[Dict[str, Any]], path: List[str], default=None):
        values = []
        ...
        for p in persona_list:
            ...
            # normalize: take first element if list
    ...
    def _collect_unique(self, persona_list: List[Dict[str, Any]], path: List[str]) -> List[str]:
        out: List[str] = []
        for p in persona_list:
            cur: Any = p
            for key in path:
    ...
    # majority vote across N personas
    def _vote_personas(self, personas: List[Dict[str, Any]]) -> Dict[str, Any]:
        if not personas:
            return {}

        final = personas[0].copy()

        final["writing_style"] = {
            "tone": self._vote_field(personas, ["writing_style", "tone"]),
            "sentence_length": self._vote_field(personas, ["writing_style", "sentence_length"]),
            "review_length": self._vote_field(personas, ["writing_style", "review_length"]),
            "filler_words": self._collect_unique(personas, ["writing_style", "filler_words"]),
        }

        final["rating_behavior"] = {
            "tendency": self._vote_field(personas, ["rating_behavior", "tendency"]),
        }

        final["emotion_signals"] = {
            "positive": self._collect_unique(personas, ["emotion_signals", "positive"]),
            "negative": self._collect_unique(personas, ["emotion_signals", "negative"]),
        }

        final["item_sentiment_prior"] = {
            "expected_sentiment": self._vote_field(
                personas, ["item_sentiment_prior", "expected_sentiment"]
            ),
        }

        final["sample_review"] = self._vote_field(personas, ["sample_review"])

        final["current_domain_expertise"] = self._vote_field(
            personas, ["current_domain_expertise"]
        )

        final["logic_patterns"] = {
            "contrast_usage": self._vote_field(personas, ["logic_patterns", "contrast_usage"]),
            "evaluation_priority": self._vote_field(personas, ["logic_patterns", "evaluation_priority"]),
            "summary_style": self._vote_field(personas, ["logic_patterns", "summary_style"]),
        }

        final["domain_interest"] = {
            "current_domain": self._vote_field(personas, ["domain_interest", "current_domain"]),
            "interest_focus": self._collect_unique(personas, ["domain_interest", "interest_focus"]),
            "utility_focus": self._vote_field(personas, ["domain_interest", "utility_focus"]),
            "aesthetic_focus": self._vote_field(personas, ["domain_interest", "aesthetic_focus"]),
            "experience_focus": self._vote_field(personas, ["domain_interest", "experience_focus"]),
        }

        return final

    # public API: build persona with sampling + voting
    def build(
        self,
        user_profile: Dict[str, Any],
        ...
        all_reviews = [r for r in user_reviews if r]
        if not all_reviews:
            return {}

        # 小样本时就不要多次 vote 了
        votes = self.persona_votes
        ...
        if self.debug:
            print(f"\n====== [Persona Builder] Start ({votes} votes) ======", flush=True)
            print(f"- Total user reviews: {len(all_reviews)}", flush=True)

        for i in range(votes):
            sampled_reviews = self._sample_reviews(all_reviews)
            ...
            pj = self._safe_json(raw)
            personas.append(pj)

        final_persona = self._vote_personas(personas)
        return final_persona


###########################################################
# 6. Reasoner (rating + review generation, single-call CoT)
###########################################################
class MyReasoner:
    ...
    def _parse_persona_emotion(self, persona_json: Any) -> Dict[str, str]:
        """
        从 persona JSON 中抽取：
        - rating_tendency / tendency
        - emotional_intensity
        - extreme_review_frequency
        （兼容旧 schema 和现在 voting 的 schema）
        """
        ...
        rb = data.get("rating_behavior", {}) if isinstance(data, dict) else {}
        es = data.get("emotional_style", {}) if isinstance(data, dict) else {}

        # 兼容 "rating_tendency" 和 "tendency"
        rating_tendency = (
            rb.get("rating_tendency")
            or rb.get("tendency")
            or rating_tendency
        )
        emotional_intensity = es.get("emotional_intensity", emotional_intensity)
        extreme_freq = es.get("extreme_review_frequency", extreme_freq)

        return {
            "rating_tendency": str(rating_tendency),
            "emotional_intensity": str(emotional_intensity),
            "extreme_review_frequency": str(extreme_freq),
        }
    ...
    def generate_review(
        ...
        """
        单次 LLM 调用里完成一个 structured reasoning 序列：
        - 解释 persona
        - 解析 domain & item context
        - 注入 rating priors
        - 推断总体 sentiment
        - 写出简洁、风格一致的 review
        """
        ...
        # persona 情绪风格
        persona_emotion = self._parse_persona_emotion(persona_json)
        rating_tendency = persona_emotion["rating_tendency"]
        emotional_intensity = persona_emotion["emotional_intensity"]
        extreme_freq = persona_emotion["extreme_review_frequency"]
        ...
        persona_guidance = f"""
Persona emotional style:
- rating_tendency: {rating_tendency}
- emotional_intensity: {emotional_intensity}
- extreme_review_frequency: {extreme_freq}

Use this to shape how strong the language should feel:
- If emotional_intensity is "low", keep wording fairly calm and measured, even for high or low ratings.
- If "medium", express satisfaction or dissatisfaction clearly, but avoid overly dramatic language.
- If "high", it is natural to show stronger enthusiasm or frustration when ratings are very high or very low.
- Only write very extreme praise or complaints when both emotional_intensity and extreme_review_frequency suggest it.
"""
...
class MySimulationAgent(SimulationAgent):
    ...
    Planning → Context Gathering → Rating Stats Analysis →
    Memory Retrieval + Domain-focused guidance →
    Persona Build (multi-sample + voting) →
    Single-call CoT Review Generation →
    Stats-based rating calibration → Final output
    """
    ...
    def __init__(self, llm: LLMBase):
        ...
        # persona_votes 默认 3，你可以改成 1 以减小开销
        self.persona_builder = PersonaBuilder(llm=self.llm, persona_votes=3, sample_size=20, debug=False)
